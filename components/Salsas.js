...
const salsas = [
  ...
  { name: "Salsa Roja", level: 2, desc: "A classic red salsa featuring roasted tomatoes, chilies, garlic, and onions—smooth, savory, and deeply flavorful with a gentle kick.", color: "#FF9800", image: "/images/roja.webp" },
  { name: "Charred Tomato Salsa", level: 3, desc: "Smoky and rich, this salsa is made with flame-roasted tomatoes, garlic, and jalapeño peppers, delivering a deep, earthy heat with rustic charm with a bit of a spiced kick.", color: "#F44336", image: "/images/roja.webp" },
  { name: "Buenos Hotcha", level: 4, desc: "A fiery red hot sauce bursting with bold chili flavor and a tangy finish—Buenos Hotcha brings the heat and attitude to anything on your plate.", color: "#B71C1C", image: "/images/roja.webp" },
];
  ...
  return (
    <motion.div 
      ...
      style={{ 
        ...
        padding: '1.5rem', 
        borderRadius: '16px', 
        borderLeft: `8px solid ${salsa.color}`, 
        boxShadow: '0 10px 30px rgba(62,39,35,0.08)',
        cursor: 'pointer'
      }}
    >
      <div style={{ flexShrink: 0, width: '120px', height: '120px', borderRadius: '50%', overflow: 'hidden', border: `4px solid ${salsa.color}`, boxShadow: '0 4px 15px rgba(0,0,0,0.1)', transform: 'translateZ(30px)' }}>
        <img src={salsa.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>

      <div style={{ flex: 1, transform: 'translateZ(20px)' }}>
        <div className="flex items-center gap-4 flex-wrap" style={{ marginBottom: '0.75rem' }}>
          <h3 className="text-foreground" style={{ fontSize: '1.5rem', fontWeight: '800', textTransform: 'uppercase' }}>{salsa.name}</h3>
          <div style={{ backgroundColor: salsa.color, color: salsa.level > 2 ? 'white' : '#3E2723', padding: '4px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '0.875rem', transform: 'translateZ(10px)' }}>
  ...
  return (
    <section id="salsas" className="py-24 relative perspective-container">
      ...
      >
        ...
        >
          <h2 className="script-font text-primary" style={{ fontSize: 'clamp(3.5rem, 6vw, 5rem)', marginBottom: '0.5rem' }}>Our Salsas</h2>
